...
export default class PhCalendar extends Component {
    ...
    componentDidMount() {
        // 计算第一个item和最后一个item的高度
        this.monthItemFirstHeight = document.getElementById('ph-c-month-first').offsetHeight
        this.monthItemLastHeight = document.getElementById('ph-c-month-last').offsetHeight
    }
    ...
    changeMonthRangeHandler(evt) {
        ...
        if(this.longTouch === true) {
            ...
            console.log('move', absY)
        }else {
            // 点击
            console.log('click', absY)
        }
    }
    //
    scrollChangeMonth(moveY){
        const wrapDom = this.refs.phContentWrap
        const contentDom = this.refs.phContent
        const wrapHeight = wrapDom.offsetHeight
        const contentHeight = contentDom.offsetHeight
        const scrollTop = wrapDom.scrollTop

        if(scrollTop < this.monthItemFirstHeight){
            console.log('need to add before')
        }
        const dis = contentHeight - scrollTop - wrapHeight
        if( dis < this.monthItemLastHeight){
            console.log('need to add after')
        }
    ...
    render(){
        const {weekStart, weekLabel} = this.props
        const {monthRange, top} = this.state
        const currentDate = monthRange[0]
        ...
        return (
            <div className="ph-c-container">
                ...
                <div className="ph-c-content-wrap" ref="phContentWrap" id="ph-content-wrap"
                     onTouchStart={::this.onTouchStartHandler}
                     onTouchMove={::this.onTouchMoveHandler}
                     onTouchEnd={::this.changeMonthRangeHandler}>
                    <div className="ph-c-content" ref="phContent" style={{'top': top + 'px'}}>
                        {
                            monthRange.map((monthItem, monthIndex)=>{
                                const year = monthItem.getFullYear()
                                const month = monthItem.getMonth()
                                return (
                                    <div className="ph-c-month" key={monthIndex} id={'ph-c-month-'+ (monthIndex == 0 ? 'first': (monthIndex == len-1 ? 'last': monthIndex))}>
